...
{
private:
  // using pool = std::unordered_map< std::size_t, class message_pool_entry >;
public:
  // using entry = pool::iterator;
  // using symbolic = message_pool_entry*;
private:
  mutable boost::recursive_mutex _rmtx;
...
public:
  ...
  void store( message::ref msg_ref, const peer::ref &peer_ref); // 受信したメッセージを追加
  void store( message::ref msg_ref, const ip::udp::endpoint &ep );
